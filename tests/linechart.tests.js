...
import { addDataset, clearDatasets } from "../src/datasets";
...
describe("linechart helpers suite", () => {
    test("extract years from country data - standard dataset", () => {
        expect(extractYears("production", countryMock1)).toStrictEqual(years);
    });

    test("extract years from country data - non-existant dataset", () => {
        expect(extractYears("test-ds", countryMock1)).toStrictEqual([]);
    });

    test("extract years from country data - empty dataset", () => {
        expect(extractYears("testdataset", countryMock1)).toStrictEqual([]);
    });


    test("extract values from country data - standard dataset", () => {
        expect(extractValues("production", countryMock1)).toStrictEqual(prodValues);
    });

    test("extract values from country data - non-existant dataset", () => {
        expect(extractValues("test-ds", countryMock1)).toStrictEqual([]);
    });

    test("extract values from country data - empty dataset", () => {
        expect(extractValues("testdataset", countryMock1)).toStrictEqual([]);
    });

    test("get dataset min max from country data - standard dataset", () => {
        expect(getDatasetMinMax("production", countryMock1))
            .toStrictEqual(prodMinMax);
    });

    test("get dataset min max from country data - empty dataset", () => {
...
describe("datasets-dependent linechart helpers suite", () => {
    ...
    test("get min max of a country - one dataset", () => {
        addDataset("production");
        expect(getMinMax(true, false, countryMock1)).toStrictEqual(prodMinMax);
    });

    test("get min max of a country - two datasets", () => {
        addDataset("production");
        expect(getMinMax(true, false, countryMock1)).toStrictEqual(prodMinMax);
    ...
    test("get min max of a country - two datasets different years", () => {
        addDataset("production");
        addDataset("import");
